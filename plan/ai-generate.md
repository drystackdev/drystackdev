...
- Parser emit *chỉ dòng dở* thay vì toàn bộ text tích luỹ ⇒ chữ đã hiện bị nuốt mất
...
| `openai-compatible` | *(không có — bắt buộc khai `DRY_AI_MODEL`)* | `DRY_AI_BASE_URL` + `/chat/completions` |
...
> 📌 Xem memory *"CF secrets are runtime-only"*: Cloudflare Pages Secrets
...
> ⚠️ Xem memory *"Editor autosave on browser test"*: khi test bằng Playwright,
...
```ts
type AiFieldKind =
  | 'text' | 'content' | 'select' | 'multiselect'
  | 'checkbox' | 'date' | 'integer' | 'url'
  | 'array' | 'object';

// Field AI không đụng tới ⇒ không hiện ở CẢ HAI cột trong dialog.
const UNSUPPORTED = ['image', 'file', 'images', 'files',
...
*"fields.timestamp() field"*, việc đóng dấu do `useUpsertItem` lo, AI chạm vào là sai.
...
- **Readonly khi đang stream**, theo memory *"VEI content useLayoutEffect wipe"* —
...
`useLayoutEffect` đã từng gây bug ở memory *"VEI content useLayoutEffect wipe"*).
...
| 7 | Memory *"astro dev --background stale dist"* | Sửa `packages/drystack/src` xong phải **build lại** `@drystack/core`, không thì dev server phục vụ `dist/` cũ. |
...
- **Field `content` có `<img>`**: theo memory *"VEI content image src format"*,
