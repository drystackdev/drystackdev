...
| `DRY_AI_KEY` | ✅ | API key |
| `DRY_AI_MODEL` | tuỳ | Bỏ trống sẽ dùng default của provider |
...
| `openai-compatible` | *(không có — phải khai `DRY_AI_MODEL`)* | `DRY_AI_BASE_URL` |
...
Sai:
```json
{"configured":false,"reason":"missing-key","message":"DRY_AI_KEY chưa được cấu hình."}
```
...
| `missing-model` | `openai-compatible` bắt buộc khai `DRY_AI_MODEL` |
