...
import { describeField, describeFields, renderSkeleton } from './schema-to-yaml';
...
test('drops an object whose children are all unsupported', () => {
  const spec = describeField(
    'tools',
    fields.object({ icon: fields.image({ label: 'Ảnh' }) }, { label: 'Công cụ' })
  );
...
test('renders a skeleton the prompt can use', () => {
  ...
  expect(skeleton).toContain('excerpt (văn bản nhiều dòng, bắt buộc): Mô tả ngắn');
  expect(skeleton).toContain('keywords (văn bản ngắn): Từ khóa SEO — Cách nhau bởi dấu phẩy');
...
test('renders nested array > object in the skeleton', () => {
  ...
  expect(skeleton).toContain('process (danh sách các mục, mỗi mục gồm): Quy trình');
