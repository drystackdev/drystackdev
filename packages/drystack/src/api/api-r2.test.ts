...
import { createUserFile, signSession, userFileKey } from './native-auth';
...
test('blob serves by sha and refuses auth/ and unknown paths', async () => {
  ...
  const authPath = await handler(
    request('GET', `blob/${sha}/auth/native/admin@example.com.json`),
    ['blob', sha, 'auth', 'native', 'admin@example.com.json']
  );
...
test('update requires a session, writes with sha metadata, deletes prefixes', async () => {
  ...
  const evil = await handler(
    request('POST', 'update', {
      body: {
        additions: [
          {
            path: 'auth/native/attacker@example.com.json',
  ...
  expect(bucket.store.has('auth/native/attacker@example.com.json')).toBe(false);
});
...
test('auth flow: status → setup → login → me → logout', async () => {
  ...
  // correct login returns the decrypted profile
  res = await handler(
    request('POST', 'auth/login', {
