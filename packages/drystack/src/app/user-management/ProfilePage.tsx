...
export function ProfilePage() {
  ...
  const api = useMemo(
    () => makeUserManagementApi(router.basePath),
    [router.basePath],
  );
  ...
  const profileState = useData(
    useCallback(() => api.getProfile(), [api, reloadKey]),
  );
  ...
  return (
    <Flex alignItems="center" gap="large">
      <label style={{ cursor: "pointer" }}>
        {user.avatar ? (
          <Avatar
            src={avatarUrl(props.basePath, user.avatar)}
            alt={user.name}
            size="xlarge"
          />
        ) : (
      ...
      <Flex direction="column" gap="regular">
        ...
        {isUploading && (
          <Text size="small">{stringFormatter.format("loading")}</Text>
        )}
      </Flex>
    </Flex>
  );
