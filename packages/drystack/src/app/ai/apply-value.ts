...
export function aiValueToFormValue(
  ...
  switch (spec.kind) {
    ...
    case "content": {
      ...
      //
      // Nothing here special-cases `<svg>`: the editor has a real `svg` node
      // now (see markdoc/editor/svg-markup.ts), so the field's own parse keeps
      // a model-drawn chart as inline markup - sanitized on the way in - and a
      // field whose schema has no such node drops it, same as it drops any
      // other tag it can't hold. Before that node existed this function had to
      // rewrite each `<svg>` into an `<img>` backed by generated bytes, which
      // is exactly what put charts outside the reach of the site's theme.
