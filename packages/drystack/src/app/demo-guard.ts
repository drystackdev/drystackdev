...
// cross-package dependency just to share one string.
const DEMO_BLOCKED_MESSAGE = 'Demo mode: changes are not saved';

export function blockWriteInDemo(): void {
  toastQueue.info(DEMO_BLOCKED_MESSAGE, { timeout: 4000 });
}
...
export function blockWriteInDemoWithError(): never {
  blockWriteInDemo();
